module model_nkmp_t
  ...
  interface nkmp_model
    module procedure new_nkmp_model
  end interface
...
contains
  ...
  end function new_nkmp_model


  subroutine system_matrices(self, para, error) 
    class(nkmp_model), intent(inout) :: self
    ...
    real(wp) :: tau,kap,psi1,psi2,rhor,rhogg,rhozz,rA,piA,gamQ,sigr,sigg,sigz
    real(wp) :: gss,piss,gam,phi,bet,nu
